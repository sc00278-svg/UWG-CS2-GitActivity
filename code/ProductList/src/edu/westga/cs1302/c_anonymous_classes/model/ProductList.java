package edu.westga.cs1302.c_anonymous_classes.model;
...
public class ProductList {
	...
	public void printProductsWithLowestPriceFirst() {
		Comparator<Product> lowestPriceComparator = new Comparator<Product>() {
			@Override
		...
		Collections.sort(tmpProducts, lowestPriceComparator);
	...
	public void printProductsWithHighestPriceFirst() {
		Comparator<Product> highestPriceComparator = new Comparator<Product>() {
			@Override
			public int compare(Product thing1, Product thing2) {
				if (thing1.getPrice() < thing2.getPrice()) {
					return 1;
		...
		List<Product> tmpProducts = new ArrayList<Product>(this.products);
		Collections.sort(tmpProducts, highestPriceComparator);
